...
class TodotxtParser:
    ...
    def exportToTracks(self, tracks_client, refetch=False):
        ...
        remote_projects = [
            remote_project["name"] for remote_project in self.remote_projects
        ]
        ...
        remote_contexts = [
            remote_context["name"] for remote_context in self.remote_contexts
        ]
        ...
        for [index, todo] in todos:
            ...
            if todo.getDescription() not in remote_todos and todo.getTracksId() == None:
                if self.verbose:
                    print(
                        "Adding local todo %s to remote Tracks instance"
                        % todo.getDescription()
                    )
        ...
        for remote_todo in self.remote_todos:
            ...
            if index_of_local_instance:
                if remote_todo["state"] == "completed":
                    ...
                    self.data["todos"][index_of_local_instance].setCompletedDate(
                        remote_todo["completed-at"][0:10]
                    )
                continue

            # Update the tid on the local todo instead of creating a new record remotely
            index_of_similar_todo = self.localTodoHasSameDescription(
            ...
            if remote_todo["state"] == "active":
                ...
                remote_todo["state"] == "deferred"
            ):  # Similar to pending but waiting to a date-time
        ...
        for index, local_todo in self.data["todos"].items():
            if local_todo.getTracksId() != None and not local_todo.isDone():
                ...
                if remote_missing:
                    ...
                    self.data["todos"][index].setCompletedDate(
                        time.strftime("%Y-%m-%d")
                    )
    ...
    def writeTodosToFile(self, todos, todo_type="todo"):
        ...
        for [index, todo] in todos.items():
            ...
            try:
                ...
                raise ValueError(
                    "line: {0}, todo.getTextLine(): {1}".format(
                        line, todo.getTextLine()
                    )
                )
        todo_file.close()
